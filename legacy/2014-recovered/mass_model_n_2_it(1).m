...
function [m0,tb1,tb2,Mp1,Mp2,M_1,M_2]=mass_model_n_2_it (DeltaVestimation,Mpl,Isp1,Isp2,OF1,OF2,T_1,T_2,V1perc,V2perc) % Eps = Epsilon = structural factor, vai ter que sair burn time para a trajectory
load all;
%  clear V1per
% clc
...
for V2per = (V2perc-0.1):0.01:(V2perc+0.1)     % ciclo for para %V1 a variar% +-10%
    ...
    for Eps2 = 0.05:0.01:0.17
        ...
        if abs(Mglobal(iiii,3,ii)-Mglobal(iiii,5,ii))<=1000
    ...
    for Eps1 = 0.05:0.01:0.17
        ...
        if abs(Mglobal(iiiii,2,ii)-Mglobal(iiiii,4,ii))<=1000
